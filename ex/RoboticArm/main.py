...
s0 = stepper(port=0, micro_steps=32, hold_current=20, run_current=20, accel_current=20, deaccel_current=20,
             steps_per_unit=200, speed=100)

s0.set_max_speed(100)
...
class MainScreen(Screen):
    ...
    def readSensor(self, i):
        ...
        return not bool(int(bin(cyprus.read_gpio())[-1 * (i + 1)]))
    ...
    def toggleMagnet(self):
        ...
        if self.magnetControl.text == "Drop Ball":
    ...
    def auto(self):
        ...
        if self.readSensor(0):
            while self.readSensor(0):
                self.armControl.text = "Lower Arm"
                cyprus.set_pwm_values(1, period_value=100000, compare_value=0,
                                      compare_mode=cyprus.LESS_THAN_OR_EQUAL)

                self.homeArm()
                self.setArmPosition(37.3)
        ...
        elif self.readSensor(1):
            while self.readSensor(1):
                self.armControl.text = "Lower Arm"
                cyprus.set_pwm_values(1, period_value=100000, compare_value=0,
                                      compare_mode=cyprus.LESS_THAN_OR_EQUAL)

                self.homeArm()
                self.setArmPosition(29)
    ...
    def setArmPosition(self, position):
        ...
        s0.goTo(int(position * 256))
        self.armControlLabel.text = 'Arm Position: ' + str(self.armPosition)
